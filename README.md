...
[Njordium Inspect360](https://njordium.com/products/inspect360/) is Njordium's SaaS-hosted platform for **vendor risk management, compliance, and financial oversight**. It positions itself as *"One Platform for Vendor Risk, Compliance and Financial Oversight"* under the tagline *"Governance-led, risk-owned, compliance-driven."* Deployment runs on customer-controlled cloud or on-premise infrastructure, emphasising data sovereignty and no vendor lock-in.

Inspect360 targets enterprise governance, risk and compliance teams. That is Risk Managers, Compliance Officers, Vendor Managers, Internal Auditors and TPRM teams. It provides:
...
- **I360: Overview**. Eight KPI tiles in a compact 2×4 grid: Approved, Drafts, Under Review and Archived vendor counts, plus Active Vendors, Total Vendors, Total Services and Total Assessments. Each tile is clickable and deep-links to the equivalent Inspect360 filter page (`/vendors?status=approved` and so on). Backed by a single call to `/api/v1/suppliers/stats` plus a lightweight products count and an assessment-array count. One round-trip pair per refresh.

#### Vendor pipeline

- **I360: Approved**. Suppliers with `status=approved`. Each row shows a green circular badge, organisation name, city and country, plus the approval date (`approved_at`) as a relative timestamp. Regulatory flag chips (Critical, ICT, Data Processor, AML) appear where relevant.
- **I360: In Progress**. The Vendor Manager's workload. Suppliers in **draft OR under_review**, sorted newest-first by `updated_at`. Colour badges automatically differentiate draft rows (grey pencil) from under-review rows (orange clock) in one list. Because the Inspect360 API does not support multi-status filtering in a single call, this endpoint fires two upstream calls and merges plus sorts server-side.
...
### Auto-refresh with tab-hidden pause
...
Full OWASP Top 10 (2021) and API Security Top 10 (2023) mapping and audit history in [`SECURITY.md`](SECURITY.md).
...
- Nextcloud **30 to 35**
- **Njordium Inspect360**. Any instance reachable from the Nextcloud host. Default is `https://ymir.njordium.io` for Njordium's demo tenant; on-prem or private-cloud instances are configurable via the admin URL field.
- PHP **8.3+** (CI verifies syntax; PHPStan runs at level 5 against `nextcloud/ocp:dev-stable30`)
- Node **22+** and npm **10+** for building from source
...
sudo -u www-data git checkout v0.4.5
...
- `lib/Controller/`. `ConfigController` (auth and admin config) plus `Inspect360APIController` (widget-facing endpoints)
...
- `lib/Settings/`. `Admin` and `Personal` settings pages
