<template>
	<div class="fgw-widget">
		...
		<div v-else-if="!items.length" class="fgw-status">
			{{ t('integration_inspect360', 'No pending reviews. Nice.') }}
		</div>
			...
			<li v-for="pr in visibleItems" :key="pr.id" class="fgw-item">
				<a
					...
					<div class="fgw-item__row">
						...
						<span class="fgw-item__title">{{ pr.title }}</span>
					</div>
		...
		<NcModal v-if="showSettings" size="normal" @close="closeSettings">
			<div class="fgw-modal">
				<h3>{{ t('integration_inspect360', 'Pending reviews — settings') }}</h3>
...
export default {
	...
	methods: {
		async fetch() {
			...
			try {
				const r = await axios.get(generateUrl('/apps/integration_inspect360/review-requests'))
				...
				if (e?.response?.status === 401) {
					...
					this.error = t('integration_inspect360', 'Failed to load pending reviews.')
				}
			} finally {
				this.loading = false
			}
